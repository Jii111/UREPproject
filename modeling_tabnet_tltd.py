# -*- coding: utf-8 -*-
"""TABNET_TLTD(1105세연)2.ipynb의 사본

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1z-b_-764P5A3t7jAiEEPyU_pLCGXTdrr

# TLTD

**사용법**
- 일단 차례대로 셀 다 실행 → 마지막 main 셀 실행하면 모델 학습 시작
- 데이터를 바꾸고 싶다면, data loader에서 데이터 경로 수정
- 기본 흐름은 (1) 데이터 이미지 변환 (2) teacher model (3) student model (4) RF classifier로 분류 예측
"""
...
for index, (train, test) in enumerate(kfold_outter.split(features, target)):
    ...
    ## best model 추출
    best_model = load_model('/content/drive/MyDrive/Colab Notebooks/Urep/최종 결과물/best_model_ganTLTD(0.76).h5')
    extract = Model(inputs=best_model.inputs, outputs=best_model.layers[-2].output)
    ...
    cm = confusion_matrix(y_finaltest, Y_pred)

    plt.figure(figsize=(10, 7))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=np.unique(y_finaltest), yticklabels=np.unique(y_finaltest))
...
for index, (train, test) in enumerate(kfold_outter.split(features, target)):
    ...
    # ROC Curve for final evaluation results
    fpr = dict()
    tpr = dict()
    roc_auc = dict()
    for i in range(n_class):
    ...
    # Micro-average ROC curve and AUC for overall multi-class performance
    all_fpr = np.unique(np.concatenate([fpr[i] for i in range(n_class)]))
    mean_tpr = np.zeros_like(all_fpr)
    for i in range(n_class):
        mean_tpr += np.interp(all_fpr, fpr[i], tpr[i])
